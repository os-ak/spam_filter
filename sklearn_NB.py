...
def classify(X_train, X_test, y_train, tfidf = True):
...
#判定精度の計測
def measuremnt():
    counts, label = extraction_from_file(5100)
    X_train, X_test, y_train, y_test = train_test_split(counts, np.c_[label], train_size=0.9, test_size = 0.1)
    del counts
    result = classify(X_train = X_train, X_test = X_test, y_train = y_train, tfidf = True)
...
if __name__ == '__main__':
    ...
    #print(measuremnt())
    doc = """
正規表現は覚えると非常に応用範囲の広い技術（ログ解析、入出力チェック）なので、体が覚えるレベルまでに達することが今後の目標です
"""
    print(classify_from_doc(doc,4200))
    #DBへの操作を確定
    connection.commit()
    #MySQL接続を終了
    connection.close()
